...
class DockerVolume:
    ...
    def project_prefix(self) -> Optional[str]:
        ...
        if '_' in self.name:
            # Try known volume suffixes first (most specific first)
            suffixes = ['_postgres_data', '_n8n_data', '_mysql_data', '_mongo_data', '_redis_data']
            
            for suffix in suffixes:
                if self.name.endswith(suffix):
                    return self.name[:-len(suffix)]
            
            # Try generic _data suffix
...
class VolumeDriftResult:
    ...
    fix_option_a: Optional[str] = None          # add name: to compose
    fix_option_b: list = field(default_factory=list)  # volume copy pairs
